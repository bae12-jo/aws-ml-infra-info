...
DLC_REPOS = [
    ...
    ("huggingface-pytorch-training",  "HuggingFace PyTorch",     "sagemaker"),
    ("huggingface-pytorch-inference", "HuggingFace Inference",   "sagemaker"),
    ("vllm",                          "vLLM",                    "ec2"),
]
...
DLC_PKG_LABELS = {
    ...
    "efa": "EFA installer", "ofi_nccl": "OFI NCCL", "python": "Python",
...
SW_KEYS = {
    ...
    "CUDNN_VERSION": "cuDNN",
    "CUDA_DRIVER_VERSION": "NVIDIA Driver",
...
def _resolve_ami(ami_id: str, region: str) -> dict:
    ...
    # pcluster official AMI: description has "nvidia-X, cuda-X, efa-X, lustre-X, dcv-X"
    sw.update(_parse_pcluster_ami_sw(description))

    # DLAMI: description has "Supported EC2 instances:..."
    # check cache for richer SW data
    for cat, entries in _cache.get("dlami_groups", {}).items():
        for d in entries:
            if d.get("ami_id") == ami_id:
                sw.update(d.get("sw", {}))
                break

    # fallback: parse name for framework version
    fw_m = re.search(r"(PyTorch|TensorFlow)\s+([\d.]+)", name)
    if fw_m and fw_m.group(1) not in sw:
        sw[fw_m.group(1)] = fw_m.group(2)

    # OS from name
    name_lower = name.lower()
    if "ubuntu" in name_lower:
        sw["OS"] = "Ubuntu 22.04" if "22.04" in name_lower else "Ubuntu 24.04" if "24.04" in name_lower else "Ubuntu"
    elif "amzn2023" in name_lower or "amazon linux 2023" in name_lower:
        sw["OS"] = "Amazon Linux 2023"
    elif "amzn2" in name_lower:
        sw["OS"] = "Amazon Linux 2"

    # pcluster version tag
    pc_ver = tags.get("parallelcluster:version")
    if pc_ver:
        sw["pcluster"] = pc_ver

    # Pre-installed component status based on AMI type
    is_pcluster = bool(pc_ver) or "parallelcluster" in name.lower()
    is_dlami_pytorch = "pytorch" in name.lower() and ("deep learning" in name.lower() or "dlami" in name.lower() or "Deep Learning" in name)
    is_dlami_base = ("deep learning base" in name.lower() or "single cuda" in name.lower()) and not is_dlami_pytorch

    # Mark components missing from AMI — container check happens at check_compatibility level
    if not is_pcluster:
        sw["__post__Slurm"] = "not included"
    if "Lustre" not in sw and not is_pcluster:
        sw["__post__Lustre"] = "not included"
    if "NCCL" not in sw:
        sw["__post__NCCL"] = "not included"
    if "OFI NCCL" not in sw:
        sw["__post__OFI NCCL"] = "not included"
    sw["__post__Pyxis"] = "not included"
    sw["__post__Enroot"] = "not included"
    # HyperPod agent is needed when using DLAMI (not pcluster official AMI)
    if not is_pcluster:
        sw["__post__HyperPod agent"] = "required for HyperPod clusters"
    # AMI type metadata for template rendering
    sw["__meta__ami_type"] = (
        "pcluster" if is_pcluster else
        "dlami_pytorch" if is_dlami_pytorch else
        "dlami_base" if is_dlami_base else
        "custom"
    )
...
def _resolve_container(image_ref: str) -> dict:
    """Parse container image ref and extract SW info from cache (NGC + DLC)."""
    tag_m = re.search(r":([^@\s]+)$", image_ref)
    tag = tag_m.group(1) if tag_m else ""

    # Check NGC cache
    for c in _cache.get("ngc", []):
        if c.get("repo", "") in image_ref:
            return {
                "name": c.get("name", ""),
                "repo": c.get("repo", ""),
                "tag": tag or c.get("latest_tag", ""),
                "sw": c.get("sw", {}),
                "is_latest": tag == c.get("latest_tag"),
            }

    # Check DLC cache
    for c in _cache.get("dlc", []):
        if c.get("repo", "") in image_ref or c.get("ecr_uri", "") in image_ref:
            sw = dict(c.get("sw", {}))
            # EFA installer bundles OFI NCCL — make it explicit
            if "EFA installer" in sw and "OFI NCCL" not in sw:
                sw["OFI NCCL"] = f"bundled in EFA installer {sw['EFA installer']}"
            return {
                "name": c.get("name", ""),
                "repo": c.get("repo", ""),
                "tag": tag or c.get("latest_tag", ""),
    ...
    return {"name": image_ref, "repo": "", "tag": tag, "sw": {}, "is_latest": None}
...
def check_compatibility(ami_id: str, container_image: str,
    ...
    try:
        ...
        # AL2 EOL warning
        ami_os = ami_sw.get("OS", "")
        if "Amazon Linux 2" in ami_os and "2023" not in ami_os:
            result["warnings"].append(
                "Amazon Linux 2 reaches end-of-life June 2025. Consider upgrading to AL2023."
            )
...
def _parse_pcluster_ami_sw(description: str) -> dict:
    ...
    for pkg, label in [
        ...
        ("efa", "EFA driver"), ("lustre", "Lustre"), ("dcv", "DCV"), ("kernel", "Kernel"),
    ]:
...
async def api_check(
    ...
    return check_compatibility(ami_id.strip(), container_image.strip(),
                               pcluster_version.strip(), region.strip())
...
async def api_regions(platform: str = "pcluster"):
    """Return regions where pcluster AMIs exist, or HyperPod supported regions."""
    try:
    ...
    if platform == "pcluster":
        # Check which regions have official pcluster AMIs
        import asyncio as _asyncio
        def _check(region):
            try:
                c = boto3.client("ec2", region_name=region)
                imgs = c.describe_images(
                    Owners=["247102896272"],
                    Filters=[{"Name":"name","Values":["aws-parallelcluster-3.*x86_64*"]}],
                )["Images"]
                return region if imgs else None
            except Exception:
                return None
        # Run checks concurrently using thread pool
        from concurrent.futures import ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=10) as pool:
            results = list(pool.map(_check, all_regions))
        return sorted([r for r in results if r])
    else:
        # HyperPod: SageMaker regions (heuristic — all standard regions)
...
async def api_recommended_amis(platform: str = "pcluster", version: str = "", region: str = "us-east-1"):
    ...
    if platform == "pcluster":
        ...
        # pcluster: only official AMIs — no DLAMI mixed in
        return result
    elif platform == "hyperpod":
        # HyperPod: recommend Single CUDA DLAMI (minimal, fast boot), Base OSS as alternative
        result = []
        for cat, entries in _cache["dlami_groups"].items():
            for d in entries:
                is_recommended = "Single CUDA" in d["label"]
                result.append({
